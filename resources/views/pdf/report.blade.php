...
<body>
    ...
    @if($pdf->isNotEmpty())
        ...
        <p style="margin-top: 50px;"> 
            <strong>
                {{-- نستخدم Glyphs لاسم المستخدم الحالي لضمان ظهور العربية بشكل صحيح --}}
                {{ $arabic->utf8Glyphs($pdf->first()->employee_name ?? auth()->user()->name) }} 
            </strong> 
            {{ $arabic->utf8Glyphs('     الموظف:') }} 
        </p>
    @else
        <p style="text-align: center; margin-top: 50px;">{{ $arabic->utf8Glyphs('لا توجد بيانات لعرضها') }}</p>
    @endif
</body>
</html>
