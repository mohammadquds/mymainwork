<nav class="bg-gray-900 shadow-md" x-data="{ mobileMenuOpen: false }" dir="rtl">
            ...
            <div class="flex items-center">
                ...
                <div class="hidden sm:flex space-x-4">
                    <a href="{{ route('home.page') }}" wire:navigate
                        class="{{ request()->routeIs('home.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} rounded-md px-3 py-2 text-sm font-bold transition">الصفحة
                        الرئيسية</a>

                    @can('user.view')
                        <a href="{{ route('user.page') }}" wire:navigate
                            class="{{ request()->routeIs('user.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} rounded-md px-3 py-2 text-sm font-bold transition">المستخدمون</a>
                    @endcan

                    @can('role.view')
                        <a href="{{ route('role.page') }}" wire:navigate
                            class="{{ request()->routeIs('role.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} rounded-md px-3 py-2 text-sm font-bold transition">الصلاحيات</a>
                    @endcan

                    @can('subscription.unactive.view')
                        <a href="{{ route('subscription.page') }}" wire:navigate
                            class="{{ request()->routeIs('subscription.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} rounded-md px-3 py-2 text-sm font-bold transition">الإشتراكات</a>
                    @endcan
    ...
    <div x-show="mobileMenuOpen" x-collapse style="display: none;" class="sm:hidden border-t border-gray-800"
        ...
        <div class="space-y-1 px-2 pb-3 pt-2">
            <a href="{{ route('home.page') }}" wire:navigate
                class="{{ request()->routeIs('home.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} block rounded-md px-3 py-2 text-base font-bold">الصفحة
                الرئيسية</a>

            @can('user.view')
                <a href="{{ route('user.page') }}" wire:navigate
                    class="{{ request()->routeIs('user.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} block rounded-md px-3 py-2 text-base font-bold">المستخدمون</a>
            @endcan

            @can('role.view')
                <a href="{{ route('role.page') }}" wire:navigate
                    class="{{ request()->routeIs('role.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} block rounded-md px-3 py-2 text-base font-bold">الصلاحيات</a>
            @endcan

            @can('subscription.unactive.view')
                <a href="{{ route('subscription.page') }}" wire:navigate
                    class="{{ request()->routeIs('subscription.page') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }} block rounded-md px-3 py-2 text-base font-bold">الإشتراكات</a>
            @endcan
