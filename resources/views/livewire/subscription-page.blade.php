             ...
             <tbody class="divide-y divide-gray-100">
                 @forelse($subscriptions as $sub)
                     <tr wire:click="openDetails({{ $sub->id }})"
                         ...
                         {{-- <td class="hidden md:table-cell p-4 text-sm text-indigo-600 font-mono">{{ $sub->phone_number }}</td> --}}
                         ...
                         <td class="hidden md:table-cell p-4 text-sm text-gray-500 font-mono">
                             {{ \Carbon\Carbon::parse($sub->start_date)->format('d M Y') }}</td>

                         <td class="p-4 text-sm text-gray-500 font-mono">
                             {{ \Carbon\Carbon::parse($sub->end_date)->format('d M Y') }}</td>
                     </tr>
                 @empty
                     <tr>
             ...
             <div class="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                 ...
                 <div class="p-6 space-y-4">
                     ...
                     <div class="grid grid-cols-2 gap-y-4 gap-x-6 text-right">
                         ...
                         {{-- <div>
                    <span class="block text-xs text-gray-400 font-bold mb-1">النوع</span>
                    <span class="font-bold text-gray-800">{{ $selectedSub->type }}</span>
                </div> --}}
